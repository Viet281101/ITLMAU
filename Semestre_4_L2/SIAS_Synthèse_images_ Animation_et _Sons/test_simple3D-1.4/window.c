 ...
 * Cet exemple introduit les textures
...
/* un identifiant pour une texture */
...
void init(void) {
  GLuint image[] = { (0xFF << 24) | 0xFF, (0xFF << 24) | (0xFF << 8) | 0xFF, (0xFF << 24) | (0xFF << 8), -1 };
  /* générer un identifiant de texture */
  glGenTextures(1, &_texId);
  assert(_texId);
  /* binder _texId comme texture bidimensionnelle */
...
void scene(GLfloat angle) {
  ...
  glUniform1i(glGetUniformLocation(_pId, "use_texture"), 0);
