...
float snoise(vec3 v)
{
	...
	/* ****************************************** */
	/* parce que la précision c'est important */
	...
	void main() {
		...
		/* intensité */
		float xfuel = 1.5 - abs(2.0*xpart - 1.0);
		/* vitesse */
		float timeSpeed = 0.5;
		float realTime = timeSpeed*time*1.5;

		vec2 coordScaled = 0.01* gl_FragCoord.xy - 0.02*vec2(offset.x,0.0);
		vec3 position = vec3(coordScaled,1.0) + vec3(7223.0,7434.0,8425.0);
		...
		vec3 displacePos = vec3(1.0,0.5,1.0)*2.4*position+realTime*vec3(0.01,-0.7,2.3);
		...
		fragColor = vec4(max(fire * 1,sparks*1.5)+smoke *4,1.0);
	}
