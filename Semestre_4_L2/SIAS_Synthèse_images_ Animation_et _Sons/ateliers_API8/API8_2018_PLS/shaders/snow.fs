...
{
	float time = temps / 10;
	const mat3 p = mat3(13.,23.5112,21.71123,21.1212,28.7312,11.9312,21.8112,14.7212,61.3934);
	...
	/* Lune */
	vec2 moonXY;
	moonXY.x = resolution.x * 0.5 + resolution.x * (resolution.y / resolution.x);
	moonXY.y = resolution.y * 0.5 + resolution.y;

	vec4 moonColor = vec4(0.0, 0.0, 0.0, 1.0);

	float dmoon = sin(3.14 / (distance(moonXY, gl_FragCoord.xy) + 1.0)) * 7.0;
	...
	/* Sortie */
	fragColor = vec4(vec3(acc),1.0) + skyColor + moonColor * 0.8;
}
