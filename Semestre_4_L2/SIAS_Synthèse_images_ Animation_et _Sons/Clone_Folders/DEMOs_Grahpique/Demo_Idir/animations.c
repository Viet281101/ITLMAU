...
void transition_circulaire(void (*a0)(int), void (*a1)(int), Uint32 t, Uint32 et, int state) {
	int vp[4], i;
	GLint tId;
	static GLuint tex[2], pId;
	switch (state) {
	case GL4DH_INIT:
		...
		pId = gl4duCreateProgram("<vs>shaders/basic.vs", "<fs>shaders/circulaire.fs", NULL);
		return;
	case GL4DH_FREE:
		if (tex[0]) {
		glDeleteTextures(2, tex);
