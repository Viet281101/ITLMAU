...
gauche_embeddings = Embeddings(gauche_corpus_path, gauche_model_path)
droite_embeddings = Embeddings(droite_corpus_path, droite_model_path)
gauche_embeddings.load()
droite_embeddings.load()

models = {"GAUCHE": gauche_embeddings.get_model(), "DROITE": droite_embeddings.get_model()}

for name, model in models.items():
	vocab_size = len(model.wv)
	word_vector = model.wv.get_vector("patrie") if "patrie" in model.wv else "Word not in vocab"
	print(f"{name} model vocab size: {vocab_size}")
	print(f"Vector for 'patrie' in {name} model: {word_vector}")
