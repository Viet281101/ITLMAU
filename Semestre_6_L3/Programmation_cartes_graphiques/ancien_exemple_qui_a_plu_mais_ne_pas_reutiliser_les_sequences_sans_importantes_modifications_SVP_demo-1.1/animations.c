...
void random_anim(int state) {
  ...
  default: /* GL4DH_DRAW */
    ...
    for(int y = 0; y < (int)gl4dpGetHeight(); ++y)
      for(int x = 0; x < (int)gl4dpGetWidth(); ++x) {
	if((gl4dmURand() * 256.0) > (double)r)
	  pixels[y * gl4dpGetWidth() + x] = -1;
	else
	  pixels[y * gl4dpGetWidth() + x] = 0;
      }
