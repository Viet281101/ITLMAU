...
/*!\brief Cette fonction est appelée quand l'audio est joué et met dans \a stream les données audio de longueur \a len.
...
void ahInitAudio(const char * file) {
	...
	if(Mix_OpenAudio(44100, AUDIO_S16LSB, 2, 1024) < 0) exit(-4);
