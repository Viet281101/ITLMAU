...
#  endif
  ...
  /*!\brief recherche la position d'une valeur donnée au sein d'un
   * vecteur. Ce dernier doit être ordonné (ordre croissant).
   *
   * \param vec le pointeur vers le vecteur sur lequel la recherche se fera.
   *
   * \param data la donnée pour laquelle une position est recherchée.
   *
   * \return la position (indice) à laquelle la donnée doit être
   * insérée afin de préserver l'ordre (croissant).
   *
   * \see vector_is_ordered
   */
  extern int        vector_find_ordered_pos(vector_t * vec, int data);
  /*!\brief insère une valeur donnée dans un vecteur à une position
   * précise.
   *
   * \param vec le pointeur vers le vecteur sur lequel l'insertion se fera.
   *
   * \param pos la position (indice) à laquelle la donnée sera insérée
   * (après décalage - complexité linéaire - si nécessaire).
   *
   * \param data la donnée à insérer.
   */
  extern void       vector_insert(vector_t * vec, int pos, int data);
  /*!\brief indique si le vecteur \a vec est trié (ordre croissant).
   *
   * \param vec le pointeur vers le vecteur à tester.
   *
   * \return une valeur non nulle (vrai) si les éléments du vecteur
   * sont ordonnés (ordre croissant), 0 (faux) sinon.
   */
  extern int        vector_is_ordered(vector_t * vec);
