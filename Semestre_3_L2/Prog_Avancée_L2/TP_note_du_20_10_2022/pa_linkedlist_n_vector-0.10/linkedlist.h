...
#  endif
  ...
  /******* BONUS : À FAIRE (ÉCRIRE LES FONCTIONS) DANS LE FICHIER linkedlist.c *******/

  /*!\brief informe si oui un non la liste chaînée passée en argument
   * est vide.
   *
   * \param llptr pointeur vers la liste chaînée de type \ref
   * linkedlist pour laquelle il est nécessaire de savoir si elle est
   * empty.
   *
   * \return une valeur différente de zéro si la liste est vide, zéro
   * sinon.
   *
   * \todo travail BONUS à faire par l'étudiant(e) : cette fonction
   * \ref ll_empty est à définir dans le fichier \ref linkedlist.c
   */
  extern int ll_empty(linkedlist_t * llptr);
  
  /*!\brief retire (supprime) et désalloue un noeud de la liste
   * chaînée.
   *
   * \param llptr pointeur vers la liste chaînée de type \ref
   * linkedlist de laquelle l'élément sera supprimé. La présence de ce
   * paramètre permet de mettre à jour le pointeur \a tail de la
   * structure au cas où le \a next du noeud à supprimer est justement
   * pointé par \a tail.
   *
   * \param pos adresse du pointeur qui directement à partir duquel il
   * faudra supprimer le noeud pointé et raccorder le reste (qu'il
   * soit NULL ou non).
   *
   * \todo travail BONUS à faire par l'étudiant(e) : cette fonction
   * \ref ll_erase est à définir dans le fichier \ref linkedlist.c
   */
  extern void ll_erase(linkedlist_t * llptr, llnode_t ** pos);
  
  /*!\brief retire (supprime/désalloue) le dernier noeud de la liste
   * chaînée avant de retourner l'élément (\a ie. la donnée du champ
   * \a data de la structure \ref llnode) qui y est contenu.
   *
   * Attention il est nécessaire que la liste ne soit pas vide, tout
   * pop effectué par l'utilisateur sans certitude sur le nombre de
   * noeuds restants est de la resposabiblité de ce dernier.
   *
   * \param llptr pointeur vers la liste chaînée de type \ref
   * linkedlist pour laquelle nous retirons et renvoyons l'élément (\a
   * ie. la donnée du champ \a data de la structure \ref llnode) contenu
   * dans le dernier noeud de la liste.
   *
   * \return la donnée du champ \a data du noeud \ref llnode retiré.
   *
   * \todo travail BONUS à faire par l'étudiant(e) : cette fonction \ref
   * ll_pop_back est à définir dans le fichier \ref linkedlist.c
   */
  extern int ll_pop_back(linkedlist_t * llptr);
  
  /*!\brief retire (supprime/désalloue) le premier noeud de la liste
   * chaînée avant de retourner l'élément (\a ie. la donnée du champ
   * \a data de la structure \ref llnode) qui y est contenu.
   *
   * Attention il est nécessaire que la liste ne soit pas vide, tout
   ...
   * linkedlist pour laquelle nous retirons et renvoyons l'élément (\a
   * ie. la donnée du champ \a data de la structure \ref llnode) contenu
   * dans le premier noeud de la liste.
   *
   * \return la donnée du champ \a data du noeud \ref llnode retiré.
   *
   * \todo travail BONUS à faire par l'étudiant(e) : cette fonction \ref
   * ll_pop_front est à définir dans le fichier \ref linkedlist.c
   */
  extern int ll_pop_front(linkedlist_t * llptr);
  extern int ll_size(llnode_t * l);
