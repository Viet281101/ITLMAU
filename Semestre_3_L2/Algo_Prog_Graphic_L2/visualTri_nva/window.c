...
/*!\brief trie a bulle le tableau passé en argument. */
static void triBulle(int * tab, int n)
{
  int i, j, c;
  for (i = 0 ; i < n - 1; i++) {
    for (j = 0 ; j < n - i - 1; j++) {
...
/*!\brief trie par insertion le tableau passé en argument. */
static void triInsertion(int *tab, int n){
  int i, j, c;
  for(i=1; i<n; i++){
    c=tab[i];
    j=i-1;
...
/*!\brief trie fusion le tableau passé en argument. */
static void fusion(int n1, int *t1, int n2, int *t2, int *t){
  int i;
  if (n1 < 1){
    copie(n2, t, t2);
    return;
...
static void triFusion(int n, int *tab, int tabT){
  if (n < 2)
    return;
...
static void copie(int n, int *dest, int *src){
  while (n--){
    *(dest++) = *(src++);
  }
}
...
static void calcul(void) {
  ...
  triBulle(_a_trier, N);
  // triInsertion(_a_trier, N);
  // triFusion(N, _a_trier, 0);
}
