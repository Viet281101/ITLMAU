...
int still_need(char *board, int pos, int offset_row, int offset_col){
    int have = 0;
                    ...
                    for (j = 0; j < space-1; j++)
                        for (k = j+1; k < space; k++) 
                        {
                            if (board[temp[j]+temp[k]-i] == ' ') 
                            {
                                ...
                                else if (space > 2 && still_need(board, temp[j]+temp[k]-i, temp[j]-i, temp[k]-i) == 1) {
                                    int l;
                                    for (l = 0; l < space; l++) {
                                        if (l == j || l == k)
                                                continue;
...
void solve(){
    ...
    printf("Résolution de la grille : \n");
    print_board();
}
